...
**Note**Â Â You can convert projects and solutions that you created with WDKÂ 8 or Windows Driver Kit (WDK)Â 8.1 to work with Windows Driver Kit (WDK)Â 10 and Visual StudioÂ 2015. Before you open the projects or solutions, run the [ProjectUpgradeTool](URL). The ProjectUpgradeTool converts the projects and solutions so that they can be built using WDKÂ 10.
...
The default Solution build configuration is **Debug** and **Win32**. In versions of the WDK prior to WindowsÂ 8, this build configuration would correspond to building a driver using an **x86 Checked Build Environment**.
...
2.  Navigate to the project directory and enter the **MSbuild** command for your target.
    ...
    **Syntax** - To specify a specific configuration and platform, use the following command syntax:
    ...
    For example, the following command builds a Universal Windows driver for the "Debug" configuration, "Win32" platform, and for WindowsÂ 10.

    ``` syntax
    msbuild /t:clean /t:build .\MyDriver.vcxproj /p:Configuration="Debug" /p:Platform=Win32 /p:TargetVersion=â€Windows10â€ /p:TargetPlatformVersion=â€10.0.10010.0â€
    ```
...
Â©Â 2015 Microsoft. All rights reserved.
